...
namespace HelperTools.Helpers
{
	public static class NumberFormatting
	{
			...
			where T : struct
		{
			...
			return string.Format("{0}{1:C2}", trailing, Convert.ToDecimal(value));
		}
		...
		public static double ToFraction<T>(this T percentage)
			where T : struct
		{
			return ToFraction(percentage, PercentageRange.RangeZeroToHunderd);
		}
		...
		public static decimal ToDecimalFraction<T>(this T percentage)
			where T : struct
		{
			return ToDecimalFraction(percentage, PercentageRange.RangeZeroToHunderd);
		}
